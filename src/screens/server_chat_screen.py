...
kv_path = os.path.join(current_dir, "..", "styles", "server_chat_screen.kv")
...
class ServerChatScreen(Screen):
    ...
    def on_enter(self):
        ...
        app.user_shared_data["user_server"].refer_server_callbacks(self.callback_functions)
        app.user_shared_data["user_server"].start()
    ...
    def send_message(self):
        ...
        if message_text:
            ...
            app = MDApp.get_running_app()
            app.user_shared_data["user_server"].message_to_send = message_text
            app.user_shared_data["user_server"].sendbtn_pressed = True
    ...
    def receive_message(self, address, message_text):
        if message_text:
            # Create a container for each message 
            message_container = MDBoxLayout(
                adaptive_height=True,  
